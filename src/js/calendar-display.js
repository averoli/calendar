...
// Create main calendar structure for a given year/month in int format
function calendarStructure(year, month) {
  const calendarSection = document.getElementById("calendar__table")
  const firstMonthDay = new Date(year, month - 1, 1,2);
  const counterDay = new Date(year, month - 1, -firstMonthDay.getDay() + 1,2);
//   calendarSection.innerHTML = ''
  ...
  for (let i = 0; i < 35; i++) {
    ...
    if (isSameMonth(counterIso, firstMonthDay.toISOString())){
        displayDayEvents(counterIso)
    }
    calendarSection.innerHTML += day({
      dayNumber: counterDay.getDate(),
      date: counterIso,
      monthNumberClass: `month__number`,
    });
    counterDay.setDate(counterDay.getDate() + 1);
    }
}

export { calendarStructure, day };
