...
import { calendarStructure, day } from "./calendar-display.js";




let data = JSON.parse(localStorage.getItem("eventInfo"));
let dataMay = data[2022][5]
...
const btnNextMonth = document.getElementById('button__next')
const btnPreviousMonth = document.getElementById('button__previous')
console.log(btnNextMonth, btnPreviousMonth)
const calendarDayNumber = Array.from(
  document.querySelectorAll("[data-day='number']")
);
const reg = {
  modal_title: /^.{4,60}$/,
...
function nextMonth(){
    const today = new Date(...todayArray)
    ...
    calendarStructure(todayArray[0],todayArray[1])
}
